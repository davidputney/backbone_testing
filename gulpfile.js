...
var paths = {
  ...
  scripts : {
    ...
    external : 'source/scripts/external/{*.js, *.json}',
...
gulp.task('concat', function() {
 //   var filterItems = filter(['!' + paths.scripts.exclude, '!' + paths.scripts.bower, '!' + paths.scripts.vendor]);
   gulp.src([paths.scripts.vendor, paths.scripts.input,'!' + paths.scripts.external, '!' + paths.scripts.bower])
   .pipe(concat('main.js'))
...
gulp.task('external', function() {
   gulp.src(paths.scripts.external)
   .pipe(gulp.dest(paths.scripts.testing))
   .pipe(minifyJS())
   .pipe(gulp.dest(paths.scripts.dist));
});
...
gulp.task('listen', function () {
    ...
    gulp.watch(paths.scripts.external).on('change', function(file) {
      gulp.start('external');
      // gulp.start('refresh');
    });
});
